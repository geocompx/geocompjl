...
```{julia}
using GeoJSON
df = GeoJSON.read(GeoJSON.write(fc)) |> DataFrame |> dropmissing!
```
...
To transform a raster to points, we can simply use the `Rasters.DimPoints` constructor that we used before in @sec-rasterizing-lines-and-polygons.
