...
#| echo: false
#| output: false
# Stupid hack to workaround QuartoNotebookRunner bug untill
# https://github.com/PumasAI/QuartoNotebookRunner.jl/pull/184 is merged
...
This chapter outlines two fundamental geographic data models --- vector and raster --- and introduces the main Julia packages for working with them.
Before demonstrating their implementation in Julia, we will introduce the theory behind each data model and the disciplines in which they predominate.
...
```{julia}
#| echo: false
#| label: fig-vector-london
#| fig-cap: Illustration of vector (point) data in which location of London (the red X) is represented with reference to an origin (the blue circle).
#| layout-ncol: 2
#| fig-subcap: 
#| - A geographic CRS with an origin at 0° longitude and latitude
#| - A projected CRS with an origin located in the sea west of the South West Peninsula. 
using CairoMakie
using GeoMakie
...
fig = Figure()
ga1 = GeoAxis(fig[1, 1]; dest = "+proj=ortho") 
lines!(ga1, GeoMakie.coastlines(), color=:grey)
scatter!(ga1, (-0.1,51.5), color=:red, marker='x', markersize=20)
scatter!(ga1, (0,0), marker=:circle, markersize=20)
display(fig)

fig = Figure()
ga1 = Axis(fig[1, 1], limits=((-100_000, 900000), (-100_000, 1200000)), aspect=DataAspect())
lines!(ga1, GO.reproject(GeoMakie.coastlines(10), source_crs=GFT.EPSG(4326), target_crs=GFT.EPSG(27700)), color=:grey)
scatter!(ga1, (530000,180000), color=:red, marker='x', markersize=20)
scatter!(ga1, (0,0), marker=:circle, markersize=20)
display(fig);
```
...
### Vector from scratch {#sec-vector-layer-from-scratch}
...
We are going to use these small raster files later on in the examples (for example, @sec-raster-to-polygons).
