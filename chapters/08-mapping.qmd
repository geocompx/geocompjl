...
From the data loaded above, we can create some more geometries.
First, we create a buffer around the `tanzania` polygon.  In order to create this buffer, we need to be in a Cartesian ("planar") CRS, and the Tanzania polygon is in lat/long space (which is "geographic" or "ellipsoidal").  
...
tanzania_neigh = filter(
  :geometry => x -> GO.intersects(x, only(tanzania_buf.geometry)), 
  GeoDataFrames.read("data/world.gpkg")
)
```
...
ax, plt = scatter(fig[1, 1], nz_height.geometry, markersize=100)
fig
```
...
fig, ax, plt = poly(nz.geometry; color = nz.Median_income)
...
The `colormap` argument can be used to specify one of countless color schemes - you can manually construct your own, or use one of the [many available color schemes in Makie](https://docs.makie.org/stable/explanations/colors).
