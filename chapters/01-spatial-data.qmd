...
new_y = Y(range(1.5, step=-0.5, length=6))
```
...
```{julia}
write("output/elev.tif", Rasters.setcrs(elev_raster, GFT.EPSG(4326));)
```
...
```{julia}
write("output/grain.tif", Raster(grain, (new_x, new_y); crs = GFT.EPSG(4326)))
```
